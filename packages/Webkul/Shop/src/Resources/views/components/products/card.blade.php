...
@pushOnce('scripts')
    ...
    >
        ...
        <div
            ...
            v-if="mode != 'list'"
        >
            <div class="relative max-h-[210px] max-w-[210px] overflow-hidden max-md:max-w-full max-md:rounded-lg max-sm:max-w-full">
                ...
                <div class="action-items bg-black">
                    <!-- Product Sale Badge -->
                    <p
                        class="absolute top-1.5 inline-block rounded-[44px] bg-red-600 px-2.5 text-sm text-white max-sm:rounded-l-none max-sm:rounded-r-xl max-sm:px-2 max-sm:py-0.5 max-sm:text-xs ltr:left-1.5 max-sm:ltr:left-0 rtl:right-2 max-sm:rtl:right-0"
                    ...
                    <div class="opacity-0 transition-all duration-300 group-hover:bottom-0 group-hover:opacity-100 max-lg:opacity-100 max-sm:opacity-100">
                        ...
                        @if (boolval(+core()->getConfigData('custom_settings.special.general.allow_user_authentication')))
                            @if (core()->getConfigData('customer.settings.wishlist.wishlist_option'))
                                <span
                                    class="absolute top-2 flex h-6 w-6 items-center justify-center rounded-full border border-zinc-200 bg-white text-lg md:hidden ltr:right-5 rtl:left-5"
                                    role="button"
        ...
        >
            <div class="group relative max-h-[258px] max-w-[250px] overflow-hidden">
                ...
                <a :href="`{{ route('shop.product_or_category.index', '') }}/${product.url_key}`">
                    <x-shop::media.images.lazy
                        class="after:content-[' '] relative min-w-[150px] bg-zinc-100 transition-all duration-300 after:block after:pb-[calc(100%+9px)] group-hover:scale-105"
                        ::src="product.base_image.medium_image_url"
                        ::key="product.id"
                        ::index="product.id"
                        width="150"
                ...
                <div class="action-items bg-black">
                    <p
                        class="absolute top-2 inline-block rounded-[44px] bg-red-500 px-2.5 text-sm text-white ltr:left-2 max-sm:ltr:left-2 rtl:right-2"
                        v-if="product.on_sale"
                    >
                        @lang('shop::app.components.products.card.sale')
                    </p>
                    ...
                    <p
                        class="absolute top-2 inline-block rounded-[44px] bg-navyBlue px-2.5 text-sm text-white ltr:left-2 max-sm:ltr:left-2 rtl:right-2"
                        v-else-if="product.is_new"
                    >
                        @lang('shop::app.components.products.card.new')
                    </p>
                    ...
                    <div class="opacity-0 transition-all duration-300 group-hover:bottom-0 group-hover:opacity-100 max-sm:opacity-100">
                        ...
                        @if (core()->getConfigData('customer.settings.wishlist.wishlist_option'))
                            <span
                                class="absolute top-2 flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded-md bg-white text-2xl ltr:right-2 rtl:left-2"
                                role="button"
                                aria-label="@lang('shop::app.components.products.card.add-to-wishlist')"
                                tabindex="0"
                                :class="product.is_wishlist ? 'icon-heart-fill text-red-600' : 'icon-heart'"
                        ...
                        @if (core()->getConfigData('catalog.products.settings.compare_option'))
                            <span
                                class="icon-compare absolute top-12 flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded-md bg-white text-2xl ltr:right-2 rtl:left-2"
                                role="button"
                                aria-label="@lang('shop::app.components.products.card.add-to-compare')"
                                tabindex="0"
                                @click="addToCompare(product.id)"
